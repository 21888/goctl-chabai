...
func (g *Generator) genLogicFunction(serviceName, goPackage, logicName string,
	...
	buffer, err := util.With("fun").Parse(text).Execute(map[string]interface{}{
		...
		"comment":      comment,
		"isAdd":        strings.HasPrefix(logicName, "Add"),
		"isAddFunc":    strings.HasPrefix(logicName, "Add"),
		"isUpdateFunc": strings.HasPrefix(logicName, "Update"),
		"isDelFunc":    strings.HasPrefix(logicName, "Del"),
		"isGetOneFunc": strings.HasPrefix(logicName, "Get"),
		"isSearchFunc": strings.HasPrefix(logicName, "Search"),
		"isClearFunc":  strings.HasPrefix(logicName, "Clear"),
		"isCountFunc":  strings.HasPrefix(logicName, "Count"),
		"isSetFunc":    strings.HasPrefix(logicName, "Set"),
	})
	if err != nil {
		return "", err
	}
